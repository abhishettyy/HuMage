  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button 
        ...
        className="text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded border border-slate-200 flex items-center gap-2 transition-colors cursor-pointer shadow-xs"
      >
        {dateString}
        <i className="ti ti-chevron-down text-slate-400 text-xs" aria-hidden="true"></i>
      </button>

      {isOpen && (
        <div className="absolute top-full left-0 mt-2 p-4 bg-white border border-slate-200 rounded-xl shadow-xl z-50 w-72 text-slate-900">
          {/* Header */}
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="font-semibold text-slate-800 text-sm">{monthString}</span>
          ...
          <div className="grid grid-cols-7 gap-1 mb-2 text-center">
            {weekDays.map(wd => (
              <div key={wd} className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{wd}</div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 gap-1">
            {days.map((d, i) => {
              ...
              if (isSelected) {
                btnClass += "bg-teal-600 text-white shadow-sm font-bold";
              } else if (isToday) {
                btnClass += "bg-teal-50 text-teal-800 border border-teal-200 hover:bg-teal-100";
              } else {
